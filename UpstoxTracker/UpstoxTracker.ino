...
#include <Preferences.h> // For storing WiFi credentials and connection counter
...
Preferences preferences; // For saving WiFi credentials and connection counter
...
const int connection_threshold = 5; // Number of successful connections to switch to alternate WiFi
int connection_counter = 0; // Counter for successful connections
...
void connectWiFi() {
  ...
  connection_counter = preferences.getInt("conn_counter", 0);
  preferences.end();
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    // Increment and save the connection counter
    preferences.begin("WiFiCreds", false);
    connection_counter++;
    preferences.putInt("conn_counter", connection_counter);
    ...
    if (connection_counter >= connection_threshold) {
      ...
      display.println("Default WiFi updated");
      display.display();
    }
    preferences.end();
  } else {
